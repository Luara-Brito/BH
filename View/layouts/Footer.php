 <footer id="contato" style="background-color: #0A0D1D; float: bootton;">
     <div class="container">
         <h2 class="text-center text-white">Contato</h2>
         <div class="row  d-flex  align-items-center">
             <div class="col-md-6">
                 <form>
                     <div class="form-group">
                         <label for="nome" class="text-white">Nome</label>
                         <input type="text" class="form-control" id="nome">
                     </div>
                     <div class="form-group">
                         <label for="email" class="text-white">Email</label>
                         <input type="email" class="form-control" id="email">
                     </div>
                     <div class="form-group">
                         <label for="mensagem" class="text-white">Mensagem</label>
                         <textarea class="form-control" id="mensagem"></textarea>
                     </div>
                     <button type="submit" class="btn btn-warning m-2">Enviar</button>
                 </form>
             </div>
             <div class="col-md-6">
                 <div class="card">
                     <div class="card-body">
                         <h5 class="card-title">Localização</h5>
                         <p class="card-text">Av. Aristoteles Fernandes Valadares (Em frente a matriz)</p>
                         <p class="card-text">Centro, Arinos - Minas Gerais</p>
                         <h5 class="card-title">Redes Sociais</h5>
                         <a href="#">
                             <p class="card-text">Whatsapp: (38) 99946-9796</p>
                         </a>
                         <a href="#">
                             <p class="card-text">Instagram: @Babeariadoheitor</p>
                         </a>
                     </div>
                 </div>
             </div>
             <h5 class="text-center text-white">Horários de funcionamento: Segunda a Sábado, das 9h às 20:30h</h5>
         </div>
     </div>
 </footer>
 <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0/dist/js/bootstrap.min.js">
 </script>
 </body>

 </html>